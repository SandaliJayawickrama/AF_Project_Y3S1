...
function ViewResearchTopic() {
    ...
    useEffect(() => {
        const getListTopics = async () => {
            try {
                const res = await axios.get("http://localhost:3100/api/rtopics")
    ...
    const deleteResearch_Topic = async(id) => {
        try {
          const res = await axios.delete(`http://localhost:3100/api/rtopics/${id}`)
      ...
      const updateResearch_Topic = async (e) => {
        e.preventDefault()
        try {
          const res = await axios.put(`http://localhost:3100/api/rtopics${isUpdating}`, {item: updateItemText})
